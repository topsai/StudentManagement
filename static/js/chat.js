...
function chat(element,imgSrc,doctextContent, sk){
    //判断传入的值，是医生还是患者
    var $user = element;
    ...
    //患者
        ...
        // console.log(sk.readyState);
        if(sk) {
            // console.log(sk);
            console.log('send message');
            sk.send(JSON.stringify({'text':$textContent, 'id':GetQueryString("id")}));
        }
        if($doctextContent){
            $box.append(createuser($doctextContent)).animate({scrollTop: $(document).height()}, 300);
        }else {
